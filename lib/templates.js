...
// The icon mark: a knot resolving into a line. Used bare (no text) as the
// favicon/app-icon context, and paired with the wordmark in the header/footer
// lockup. Keep this path in sync with dpdp-untangled-brand.md if it ever changes.
const ICON_PATH = 'M10,38 C5,28 20,22 16,33 C12,43 27,39 24,28 C21,16 38,20 34,31 L48,24';

function iconSvg(strokeWidth) {
  return `<svg class="brand-icon" viewBox="0 0 56 56" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false"><path d="${ICON_PATH}" fill="none" stroke="#76E000" stroke-width="${strokeWidth}" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
}

const FAVICON = `data:image/svg+xml,${encodeURIComponent(
  `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><rect width="64" height="64" rx="14" fill="#000000"/><g transform="translate(4,4)"><path d="${ICON_PATH}" fill="none" stroke="#76E000" stroke-width="7" stroke-linecap="round" stroke-linejoin="round"/></g></svg>`
)}`;
...
<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Fraunces:ital,wght@0,400;0,500;0,600;1,400;1,500&family=Inter:wght@400;500;600;700;800&family=Syne:wght@700;800&display=swap" />`;
...
module.exports = { layout, esc, breadcrumbHtml, breadcrumbJsonLd, relatedLinksHtml, NAV, cfg, ICON_PATH, iconSvg };
